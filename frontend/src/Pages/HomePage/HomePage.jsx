...
export default function HomePage() {
  ...
  const categoryToWriteOut = categories.map(
    (category) =>
  ...
  const getCategoryAndDifficultySearchParam = () => {
    if (difficulty !== "" && chosenCategory.length !== 0) {
      ...
      let str = `category=${chosenCategory[0]
        .toUpperCase()
        .replace(/ /g, "_")}`;
      for (let i = 1; i < chosenCategory.length; i++) {
        str += `&category=${chosenCategory[i]
          .toUpperCase()
          .replace(/ /g, "_")}`;
      }
      return str;
  ...
  const getCategoryAndDifficultySearchParamForMulti = () => {
    if (difficulty !== "" && chosenCategory.length !== 0) {
      ...
      let str = `categories=${chosenCategory[0]
        .toUpperCase()
        .replace(/ /g, "_")}`;
      for (let i = 1; i < chosenCategory.length; i++) {
        str += `&categories=${chosenCategory[i]
          .toUpperCase()
          .replace(/ /g, "_")}`;
      }
      return str;
  ...
  async function createNewGame(urlParams) {
    ...
    const res = await fetch(
      `${baseUrl}?createdBy=${getPlayerId()}&${urlParams}`,
      {
        method: "POST",
      }
    );
    ...
    <>
      ...
      <Container align="center" sx={{ padding: "5rem" }}>
        <FormControl sx={{ m: 1, minWidth: 300 }}>
          <InputLabel id="difficulty-choosing">Difficulty</InputLabel>
          <Select
        ...
        <FormControl sx={{ m: 1, width: 300 }}>
          ...
          <Select
            ...
            input={
              <OutlinedInput id="select-multiple-category" label="categories" />
            }
